...
from chat.redis import StreamReader

bolt_api = BoltAPI(prefix="/api/v1", django_middleware=True)


@bolt_api.get("/chat/{chat_uuid}")
@login_required
async def chat_sse(request, chat_uuid: str, last_seq_id: Optional[int] = None):
    ...
    stream_reader = StreamReader(chat_uuid=chat_uuid, last_seq_id=last_seq_id)
